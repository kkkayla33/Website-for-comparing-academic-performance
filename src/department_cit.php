<?Php
    ...
    $stmt=$dbcon->prepare("SELECT `Department`.`title`,AVG(citation) as citation, MAX(citation) as max_citation,MIN(citation) as min_citation
...
?>
    ...
    <div class="row justify-content-around">
        ...
        <div class="col-md-9">
            <div class="jumbotron">
                <h3>Citation Comparison By Department </h1>
                ...
                <div class="container">
                    <div class="row">
                        <div class="col-md-9">
                            ...
                            <br> 
                            <?php
                            $sql = "SELECT `Department`.`title`,AVG(citation) as citation, MAX(citation) as max_citation,MIN(citation) as min_citation
                            FROM Faculty,`Faculty-Department`,Department
                            WHERE `Faculty`.`f_id` = `Faculty-Department`.`f-id`
                                AND `Department`.`d-id` = `Faculty-Department`.`d-id`
                            GROUP BY `Faculty-Department`.`d-id`";
                            ...
                            echo "<table>
                                  <tr><th>School</th><th>Average citation</th><th>Max citation</th>
                                  </tr>";
                            // output data of each row
                            while($row = $result->fetch(PDO::FETCH_ASSOC)) {
                                echo "<tr><td>" . $row["title"]. "</td>". 
                                        "<td>" . $row["citation"]. "</td>".
                                        "<td>" . $row["max_citation"]. "</td></tr>"
                                ;
                            }
                            echo "</table>";
                            ?>
